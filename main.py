...
def showAccelerometer():
    ...
    def update_graph():
        ...
        acceleration = ser.readline().decode().strip()
...
def showAltimeter():
    ...
    # Configuração da comunicação serial
    ser = serial.Serial('/dev/cu.usbserial-14440', 57600)
...
def showGyroscope():
    ...
    def update_graph():
        ...
        gyro_value = ser.readline().decode().strip()
        timestamp = datetime.datetime.now()
...
def showSpeedometer():
    ...
    def update_graph():
        ...
        speed = ser.readline().decode().strip()
        timestamp = datetime.datetime.now()
